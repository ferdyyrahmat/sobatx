...
public function login()
{
   ...
   echo view('Backend/master-admin-peternak/auth/login', $data);
}
...
if($cekToko != 0){
   ...
   $dataPesanan = $modelPesanan->getDataPesananJoin(['tbl_pesanan.id_toko' => session('id_toko')])->getResultArray();
...
//    $dataTransaksi= $modelTransaksi->getDataTemp(['id_transaksi' => $idView])->getRowArray();

//    $idUpdate = $dataTransaksi['id_transaksi'];

//    $dataUpdate = [
//       'status' => '1',
//       'updated_at' => date('Y-m-d H:i:s')
//    ];
//    $whereUpdate = ['id_transaksi' => $idUpdate];
